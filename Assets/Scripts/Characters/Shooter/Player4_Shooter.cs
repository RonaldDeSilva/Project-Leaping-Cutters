...
public class Player4_Shooter : MonoBehaviour
{
    #region Hinge
    ...
    private void Awake()
    {
        ...
        lives = int.Parse(Can.transform.GetChild(3).gameObject.GetComponent<Text>().text);
        ...
        if (Respawn == null)
        {
            Respawn = GameObject.Find("Respawn4").transform;
        }
        #endregion
    }
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetAxis("Dash4") > 0 && !Dashed && !Dashing && !Recoiling)
        {
            StartCoroutine("Dash");
        }
        ...
        if (Input.GetAxis("Shoot4") > 0 && !Recoiling && !Reloading && !Dashing)
        {
    ...
    IEnumerator Shoot()
    {
        RecoilDir = new Vector2(-Input.GetAxis("Horizontal4") * RecoilSpd, Input.GetAxis("Vertical4") * RecoilSpd);
        ProjDir = new Vector2(Input.GetAxis("Horizontal4") * ProjSpd, -Input.GetAxis("Vertical4") * ProjSpd);
