...
public class Player3Extender : MonoBehaviour
{
    #region Hinge
    ...
    private void Awake()
    {
        ...
        lives = int.Parse(Can.transform.GetChild(2).gameObject.GetComponent<Text>().text);
    ...
    void FixedUpdate()
    {
        ...
        if (isArmStationary)
        {
            if (!Extending && !Retracting)
            {
                ...
                var controllerRot = new Vector3(0, 0, (Mathf.Atan2(-Input.GetAxis("Vertical3"), Input.GetAxis("Horizontal3")) * 180 / Mathf.PI) - 90f);
                ...
                if (Input.GetAxis("Vertical3") > 0.15 || Input.GetAxis("Vertical3") < -0.15 || Input.GetAxis("Horizontal3") > 0.15 || Input.GetAxis("Horizontal3") < -0.15)
                {
                ...
                if (Input.GetAxis("Dash3") > 0 && !Dashed && !Dashing && !Retracting && !Extending)
                {
                    StartCoroutine("Dash");
                }
            ...
            if (Input.GetAxis("Shoot3") > 0 && !Extending && !Extended && !Retracting && !Dashing)
            {
        ...
        else if (!isArmStationary)
        {
            ...
            //Input for the dash ability
            if (Input.GetAxis("Dash3") > 0 && !Dashed && !Dashing && !Extending && !Retracting)
            {
                StartCoroutine("Dash");
            }
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        if (collision.CompareTag("Deathbox"))
        {
            ...
            {
                lives -= 1;
                Can.gameObject.transform.GetChild(2).gameObject.GetComponent<Text>().text = lives.ToString();
                Instantiate(Player, Respawn.position, this.transform.rotation);
                Can.transform.GetChild(2).GetChild(1).gameObject.GetComponent<SpriteRenderer>().color = Color.blue;
                Can.transform.GetChild(2).GetChild(2).gameObject.GetComponent<SpriteRenderer>().color = Color.blue;
    ...
    IEnumerator Dash()
    {
        ...
        Dashing = true;
        Can.transform.GetChild(2).GetChild(1).gameObject.GetComponent<SpriteRenderer>().color = Color.red;
    ...
    IEnumerator Extend()
    {
        ...
        Extending = true;
        Can.transform.GetChild(2).GetChild(2).gameObject.GetComponent<SpriteRenderer>().color = Color.red;
