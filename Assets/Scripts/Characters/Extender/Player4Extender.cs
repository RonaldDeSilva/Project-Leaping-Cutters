...
public class Player4Extender : MonoBehaviour
{
    #region Hinge
    ...
    void FixedUpdate()
    {
        ...
        if (isArmStationary)
        {
            if (!Extending && !Retracting)
            {
                ...
                if (Input.GetAxis("Dash4") > 0 && !Dashed && !Dashing && !Retracting && !Extending)
                {
                    StartCoroutine("Dash");
                }
            ...
            if (Input.GetAxis("Shoot4") > 0 && !Extending && !Extended && !Retracting && !Dashing)
            {
        ...
        else if (!isArmStationary)
        {
            ...
            //Input for the dash ability
            if (Input.GetAxis("Dash4") > 0 && !Dashed && !Dashing && !Extending && !Retracting)
            {
                StartCoroutine("Dash");
            }
